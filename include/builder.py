...
from .config_loader import resolve_brand_phones, validate_brand, validate_phones
...
def run_build(
    ...
    for brand_name, brand_cfg in brands.items():
        ...
        total_shots += len(phone_list) * len(brand_cfg.get("screenshots", []) or [])
    ...
    for brand_name, brand_cfg in brands.items():
        ...
        for phone_name, phone_cfg in phone_list:
            # Merge: brand-level options + phone-provided base_image/corners.
            merged_brand = {**brand_cfg, **phone_cfg}

            for shot_cfg in brand_cfg.get("screenshots", []) or []:
                counter += 1
                out_name = _output_filename(shot_cfg, phone_name, multi_phone)
                out_path = brand_out / out_name
                tag = f"{brand_name} :: {out_name}"
                if phone_name:
                    tag = f"{brand_name} [{phone_name}] :: {out_name}"
